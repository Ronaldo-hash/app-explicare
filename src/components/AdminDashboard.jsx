...
export function AdminDashboard({ supabase, session, onLogout }) {
    ...
    const [deleteClientConfirmation, setDeleteClientConfirmation] = useState('');
    ...
    const handleDeleteClient = async () => {
        if (deleteClientConfirmation !== selectedClient) {
            showToast('error', 'Nome do cliente incorreto.');
            return;
        }

        setIsDeleting(true);
        // Find all videos for this client to delete them (or let DB cascade if set, but we usually manual delete for safety/storage)
        // With basic deletion:
    ...
    const handleDeleteProcess = async () => {
        ...
        if (error) {
            ...
            showToast('error', 'Erro ao excluir processo.');
        } else {
            showToast('success', 'Processo excluído com sucesso.');
            ...
            <div className="animate-fade-in">
                            ...
                            {editingClient ? (
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    ...
                                    <div className="py-2 px-3 bg-zinc-800/30 rounded-lg flex items-center justify-between">
                                        ...
                                        <button onClick={() => setEditingClient(true)} className="text-[#c9a857] hover:text-white transition-colors ml-2" title="Editar Informações">
                                            <Info size={14} />
                                        </button>
                                        <button onClick={() => { setDeleteClientConfirmation(''); setShowDeleteClientModal(true); }} className="text-red-500 hover:text-red-400 transition-colors ml-2" title="Excluir Cliente">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-col md:flex-row gap-4">
                                ...
                                <div className="flex-1 flex flex-col justify-between">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        <button onClick={() => { setProcessToDelete(video); setShowDeleteProcessModal(true); }} className="btn-secondary text-xs py-2 px-4 flex items-center gap-2 text-red-500 border-red-500/30 hover:bg-red-500/10" title="Excluir Processo">
                                            <Trash2 size={12} />
                                        </button>
                                    </div>
                                </div>
                ...
                {showDeleteClientModal && (
                    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 animate-fade-in">
                        <div className="bg-zinc-900 border border-red-500/30 rounded-xl p-6 w-full max-w-md shadow-2xl relative">
                            <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
                                <Trash2 className="text-red-500" /> Excluir Cliente
                            </h3>
                            <p className="text-zinc-400 text-sm mb-4">
                                Esta ação é irreversível. Todos os processos e vídeos deste cliente serão apagados permanentemente.
                            </p>
                            <div className="mb-6">
                                <label className="text-xs text-zinc-500 uppercase mb-2 block">Digite <strong>{selectedClient}</strong> para confirmar:</label>
                                <input
                                    type="text"
                                    value={deleteClientConfirmation}
                                    onChange={(e) => setDeleteClientConfirmation(e.target.value)}
                                    className="input-premium w-full border-red-500/30 focus:border-red-500"
                            ...
                            <div className="flex gap-3 justify-end">
                                ...
                                <button
                                    ...
                                    disabled={deleteClientConfirmation !== selectedClient || isDeleting}
                ...
                {showDeleteProcessModal && (
                    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 animate-fade-in">
                        <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-6 w-full max-w-sm shadow-2xl">
                            <h3 className="text-lg font-bold text-white mb-2">Excluir Processo?</h3>
                            <p className="text-zinc-400 text-sm mb-6">
                                Você tem certeza que deseja excluir o processo <strong>{processToDelete?.processo}</strong>? Esta ação não pode ser desfeita.
                            </p>
                            <div className="flex gap-3 justify-end">
                                <button
                                    onClick={() => setShowDeleteProcessModal(false)}
                                    className="px-4 py-2 text-sm text-zinc-400 hover:text-white transition-colors"
                                >
                                    Cancelar
                                </button>
